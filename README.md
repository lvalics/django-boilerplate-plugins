...
| `multi_domain` | free | Serve multiple branded domains from one project: database-driven site configuration (branding, themes, feature flags), per-site templates with a site-aware template cache, per-site CORS/CSRF origins, dynamic `ALLOWED_HOSTS`, optional cross-domain SSO, audit logging, and a DRF site/member API. See `plugins/multi_domain/README.md`. |
| `cms` | free | Zone-based CMS scoped per site: landing pages, static content pages and a blog (posts, categories, tags) built on 20+ zone types with DaisyUI templates and 90+ seed presets, testimonials with Schema.org markup, order/contact form submissions with Turnstile + rate limiting, and cached slug resolution. Mounts at the project root with reserved-slug protection. Requires `multi_domain` (apps.sites). See `plugins/cms/README.md`. |
