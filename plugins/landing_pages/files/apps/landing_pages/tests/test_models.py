...
from apps.landing_pages.models import LandingPage, LandingPageZone, ZoneType
...
class LandingPageSlugConstraintTest(TestCase):
    def setUp(self):
    ...
    def test_same_slug_allowed_on_different_sites(self):
        LandingPage.objects.create(title="A", slug="promo", site=self.site_a)
        LandingPage.objects.create(title="B", slug="promo", site=self.site_b)
        self.assertEqual(LandingPage.objects.filter(slug="promo").count(), 2)

    def test_duplicate_slug_on_same_site_rejected(self):
        LandingPage.objects.create(title="A", slug="promo", site=self.site_a)
        with self.assertRaises(IntegrityError):
            LandingPage.objects.create(title="A2", slug="promo", site=self.site_a)

    def test_duplicate_all_sites_slug_rejected(self):
        LandingPage.objects.create(title="G", slug="promo")
        with self.assertRaises(IntegrityError):
            LandingPage.objects.create(title="G2", slug="promo")

    def test_for_site_prefers_site_specific_page(self):
        global_page = LandingPage.objects.create(title="G", slug="promo")
        site_page = LandingPage.objects.create(title="A", slug="promo", site=self.site_a)

        resolved = LandingPage.for_site(self.site_a.site_id).filter(slug="promo").first()
        self.assertEqual(resolved.pk, site_page.pk)

        # Other sites only see the all-sites page.
        resolved_b = LandingPage.for_site(self.site_b.site_id).filter(slug="promo").first()
        self.assertEqual(resolved_b.pk, global_page.pk)

    def test_for_site_excludes_other_sites_pages(self):
        LandingPage.objects.create(title="A", slug="only-a", site=self.site_a)
        self.assertIsNone(LandingPage.for_site(self.site_b.site_id).filter(slug="only-a").first())


class LandingPageZoneValidationTest(TestCase):
    def setUp(self):
        self.page = LandingPage.objects.create(title="Page", slug="page")

    def _zone(self, **kwargs):
        kwargs.setdefault("landing_page", self.page)
        kwargs.setdefault("zone_type", ZoneType.HERO_VIDEO)
        return LandingPageZone(**kwargs)

    def test_template_name_outside_whitelist_rejected(self):
        for bad in (
            "web/base.html",
            "landing_pages/zones/../../../settings.html",
            "landing_pages/zones/Evil.html",
            "admin/index.html",
        ):
    ...
    def test_template_name_inside_whitelist_accepted(self):
        zone = self._zone(template_name="landing_pages/zones/hero_video.html", content={"headline": "x"})
    ...
    def test_get_template_name_ignores_non_whitelisted_override(self):
        ...
        self.assertEqual(zone.get_template_name(), "landing_pages/zones/hero_video.html")
