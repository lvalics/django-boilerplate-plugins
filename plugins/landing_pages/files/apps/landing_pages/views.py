...
from .cache import get_landing_page_id
from .email_utils import validate_and_normalize_email
from .emails import send_submission_notification
from .models import LandingPage, LandingPageSubmission, LandingPageZone, SubmissionStatus, ZoneType
...
def queryset_for_request(request):
    ...
    """
    return LandingPage.for_site(_current_site_id(request))
...
def landing_page_view(request, slug):
    ...
    page_id = get_landing_page_id(slug, site_id)
    if page_id is None:
    ...
    active_zones = Prefetch(
        "zones",
        queryset=LandingPageZone.objects.filter(is_active=True).order_by("order"),
        to_attr="active_zones",
    )
    page = get_object_or_404(
        LandingPage.objects.select_related("site").prefetch_related(active_zones),
        pk=page_id,
        is_active=True,
    )
    ...
    if redirect_zone:
        ...
        if redirect_url:
            ...
            if delay > 0:
                ...
                return render(
                    request,
                    "landing_pages/zones/redirect.html",
                    {
                        "zone": redirect_zone,
    ...
    if page.use_site_template:
        template = "landing_pages/landing_page.html"
    else:
        template = "landing_pages/landing_page_standalone.html"
...
def _validate_upload(uploaded_file) -> tuple[bool, str]:
    ...
    max_size_mb = getattr(settings, "LANDING_PAGES_MAX_UPLOAD_MB", 10)
    allowed_types = getattr(settings, "LANDING_PAGES_ALLOWED_UPLOAD_TYPES", DEFAULT_ALLOWED_UPLOAD_TYPES)
...
def _store_uploads(request, zone) -> list[dict]:
    ...
    """
    uploaded_files = []
    folder = f"landing_pages/submissions/{zone.landing_page.slug}/{zone.pk}"
...
def submit_order_form(request, zone_id):
    ...
    zone = get_object_or_404(
        LandingPageZone.objects.select_related("landing_page"),
        id=zone_id,
    ...
    try:
        ...
        submission = LandingPageSubmission.objects.create(
