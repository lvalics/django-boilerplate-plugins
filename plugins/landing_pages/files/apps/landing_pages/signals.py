...
from .models import LandingPage, LandingPageZone


def _invalidate_for_page(page):
    from .cache import invalidate_landing_page_cache
    ...
    if page.site_id:
        try:
            ...
            site_id = None
    invalidate_landing_page_cache(page.slug, site_id)


@receiver(post_save, sender=LandingPage)
def invalidate_landing_page_cache_on_save(sender, instance, **kwargs):
    """Invalidate cache when a landing page is saved."""
    _invalidate_for_page(instance)


@receiver(post_delete, sender=LandingPage)
def invalidate_landing_page_cache_on_delete(sender, instance, **kwargs):
    """Invalidate cache when a landing page is deleted."""
    _invalidate_for_page(instance)


@receiver(post_save, sender=LandingPageZone)
...
@receiver(post_delete, sender=LandingPageZone)
