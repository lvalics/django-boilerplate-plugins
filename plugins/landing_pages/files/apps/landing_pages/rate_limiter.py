...
class RateLimiter:
    ...
    - LANDING_PAGES_RATE_LIMIT_IP_REQUESTS: Max requests per IP (default: 2)
    - LANDING_PAGES_RATE_LIMIT_IP_WINDOW: IP window in seconds (default: 3600)
    - LANDING_PAGES_RATE_LIMIT_EMAIL_REQUESTS: Max requests per email (default: 3)
    - LANDING_PAGES_RATE_LIMIT_EMAIL_WINDOW: Email window in seconds (default: 86400)
    - LANDING_PAGES_EXEMPT_IPS: List of IPs exempt from rate limiting
    """

    def __init__(self):
        self.ip_requests = getattr(settings, "LANDING_PAGES_RATE_LIMIT_IP_REQUESTS", 2)
        self.ip_window = getattr(settings, "LANDING_PAGES_RATE_LIMIT_IP_WINDOW", 3600)
        self.email_requests = getattr(settings, "LANDING_PAGES_RATE_LIMIT_EMAIL_REQUESTS", 3)
        self.email_window = getattr(settings, "LANDING_PAGES_RATE_LIMIT_EMAIL_WINDOW", 86400)
        self.exempt_ips = getattr(settings, "LANDING_PAGES_EXEMPT_IPS", [])

    def _get_cache_key(self, key_type: str, identifier: str) -> str:
        """Generate a cache key for rate limiting."""
        return f"landing_pages:rate_limit:{key_type}:{identifier}"
