...
- ``landing_pages:version:all`` is bumped when an all-sites page changes; it is
  part of every lookup key, so bumping it invalidates every cached resolution.
- ``landing_pages:version:<site_id>`` is bumped when a site-scoped page
...
CACHE_TIMEOUT = getattr(settings, "LANDING_PAGES_CACHE_TIMEOUT", 300)

_MISS = 0  # sentinel id stored for "slug does not resolve" (page ids are >= 1)


def _version_key(site_id) -> str:
    return f"landing_pages:version:{site_id or 'all'}"
...
def _page_key(slug: str, site_id) -> str:
    ...
    return f"landing_pages:page:g{global_version}:s{site_version}:{site_id or 'all'}:{slug}"


def get_landing_page_id(slug: str, site_id=None):
    """
    Resolve a slug (within the given Django Site id scope) to a LandingPage id.
    ...
    page = LandingPage.for_site(site_id).filter(slug=slug).only("id").first()
...
def invalidate_landing_page_cache(slug: str, site_id=None) -> None:
    """
