...
class PageViewTest(TestCase):
    ...
    def _url(self, slug):
        return reverse("cms:page", kwargs={"slug": slug})
...
class SubmitOrderFormTest(TestCase):
    def setUp(self):
        ...
        self.url = reverse("cms:submit_form", kwargs={"zone_id": self.zone.pk})

    @override_settings(TURNSTILE_SECRET="", CMS_RATE_LIMIT_IP_REQUESTS=100)
    ...
    @override_settings(TURNSTILE_SECRET="", CMS_RATE_LIMIT_IP_REQUESTS=1)
    ...
    @override_settings(TURNSTILE_SECRET="sekrit", CMS_RATE_LIMIT_IP_REQUESTS=100)
    def test_turnstile_enforced_when_secret_set(self):
        ...
        with mock.patch("apps.cms.views.requests.post") as post:
