...
class BlogPublishGatingTest(TestCase):
    def test_published_post_listed_and_detail_ok(self):
        _post("hello")
        listing = self.client.get(reverse("cms_blog:blog_list"))
        self.assertEqual(listing.status_code, 200)
        self.assertContains(listing, "hello")
        detail = self.client.get(reverse("cms_blog:blog_detail", kwargs={"slug": "hello"}))
        self.assertEqual(detail.status_code, 200)

    def test_future_published_at_is_404(self):
        _post("future", published_at=timezone.now() + timedelta(days=3))
        self.assertEqual(
            self.client.get(reverse("cms_blog:blog_detail", kwargs={"slug": "future"})).status_code,
            404,
        )
    ...
    def test_unpublished_post_is_404(self):
        ...
        self.assertEqual(
            self.client.get(reverse("cms_blog:blog_detail", kwargs={"slug": "draft"})).status_code,
            404,
        )
    ...
    def test_inactive_post_is_404(self):
        ...
        self.assertEqual(
            self.client.get(reverse("cms_blog:blog_detail", kwargs={"slug": "gone"})).status_code,
            404,
        )
...
class BlogTaxonomyFilterTest(TestCase):
    ...
    def test_category_filter(self):
        resp = self.client.get(reverse("cms_blog:blog_category", kwargs={"slug": "news"}))
        self.assertEqual(resp.status_code, 200)
        self.assertContains(resp, "in-cat")
        self.assertNotContains(resp, "other")

    def test_tag_filter(self):
        resp = self.client.get(reverse("cms_blog:blog_tag", kwargs={"slug": "django"}))
        self.assertEqual(resp.status_code, 200)
        self.assertContains(resp, "in-cat")
        self.assertNotContains(resp, "other")
...
class BlogPaginationTest(TestCase):
    def test_pagination_splits_pages(self):
        ...
        page2 = self.client.get(reverse("cms_blog:blog_list"), {"page": 2})
...
class BlogPerSiteVisibilityTest(TestCase):
    ...
    def _get(self, slug, profile):
        return self.client.get(
            reverse("cms_blog:blog_detail", kwargs={"slug": slug}),
...
class ContentPageTest(TestCase):
    def test_content_page_renders(self):
        ...
        resp = self.client.get(reverse("cms_blog:content_page", kwargs={"slug": "about"}))
        self.assertEqual(resp.status_code, 200)

    def test_landing_page_not_served_as_content(self):
        Page.objects.create(title="Promo", slug="promo", page_type=PageType.LANDING)
        self.assertEqual(
            self.client.get(reverse("cms_blog:content_page", kwargs={"slug": "promo"})).status_code,
            404,
        )
