...
templates/cms/tailwind-safelist.html, which is enforced by a repo
...
# enum is listed literally in templates/cms/tailwind-safelist.html.
ALLOWED_COLORS = {
    "primary",
    "secondary",
    "accent",
...
def _validated_color(value, default, allowed=None, allow_opacity=False):
    ...
    if value not in _warned_colors:
        ...
        logger.debug("cms: unknown color %r, falling back to %r", value, default)
    return default
...
@register.inclusion_tag("cms/components/zone_wrapper.html")
...
@register.inclusion_tag("cms/components/schema_markup.html")
